...
export const recipeTools: Anthropic.Tool[] = [
  {
    name: 'get_recipe',
...
export function getRecipeResult(dishName: string): string {
  const key = Object.keys(RECIPES).find((k) => dishName.includes(k))
  if (!key) {
    return JSON.stringify({ dish_name: dishName, ingredients: ['재료 정보 없음 — 일반적인 레시피를 안내합니다'] })
  }
  const r = RECIPES[key]
  return JSON.stringify({ dish_name: key, ingredients: r.ingredients, servings: r.servings })
}

export function buildSystemPrompt(ingredients?: { name: string; qty: string; unit: string }[]) {
